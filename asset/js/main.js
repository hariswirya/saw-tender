$(document).ready(function () {
    ...
    $('#isiDetail').load("./proses/proseslihat.php/?op=detail");
    $('#pilihDetail').change(function() {
        var value =$(this).val();
        $('#isiDetail').hide().load("./proses/proseslihat.php/?op=detail&id="+value).fadeIn('400');
    });
    $("#pilihBobot").change(function() {
        var value=$(this).val();
        $("#valueBobot").hide("slow");
        document.cookie="pilih="+value+";expires=3600;path=/";
        if (getCookieData) {
            $("#valueBobot").load("./bobotkriteria.php").slideToggle("slow");
